...
def test_make_json_safe_handles_nested_moments_payload_values():
    ...
    invalid_path = "/data/buckets/feel/\udcb5\udca6/dd1e83635bef.md"
    payload = {
    ...
    assert safe["path"] == str(Path(invalid_path)).replace("\udcb5", "\ufffd").replace("\udca6", "\ufffd")
    assert safe["普通中文"] == "保持不变"
...
def test_api_moments_serializes_the_complete_payload_before_responding():
    ...
    async def inspection_payload(**_kwargs):
        invalid_path = Path("/data/buckets/feel/\udcb5/source.md")
        return {
            "status": "ok",
    ...
    assert "\ufffd" in response.content["moments"][0]["source_window"]
    assert response.content["edges"][0]["created_at"] == "2026-07-20T09:45:00"
    assert response.content["edges"][0]["labels"] == ["anchor"]
    json.dumps(response.content, ensure_ascii=False).encode("utf-8")
